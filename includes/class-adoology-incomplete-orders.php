...
class Adoology_Incomplete_Orders {
    ...
    public static function register_routes() {
        register_rest_route('adoology/v1', '/checkout', array(
            'methods'             => WP_REST_Server::CREATABLE,
            'callback'            => array(__CLASS__, 'capture'),
            'permission_callback' => '__return_true',
        ));
        register_rest_route('adoology/v1', '/checkout-token', array(
            'methods'             => WP_REST_Server::CREATABLE,
            'callback'            => array(__CLASS__, 'issue_capture_token'),
            'permission_callback' => '__return_true',
        ));
    }
    ...
    public static function enqueue_checkout_tracker() {
        ...
        self::enqueue_tracker('checkout', array(
            'items'       => $items,
     ...
     */
    public static function enqueue_tracker($flow, $extra = array()) {
        wp_enqueue_script(
        ...
        wp_localize_script('adoology-checkout-tracker', 'adoologyCheckout', array(
    ...
    public static function capture($request) {
        ...
        if (!self::allow_capture_request()) {
            return new WP_Error('adoology_capture_limited', __('Too many checkout updates.', 'adoology-connector'), array('status' => 429));
        }

        $params = $request->get_json_params();
        if (!is_array($params)) {
            return new WP_Error('adoology_capture_invalid', __('Invalid checkout data.', 'adoology-connector'), array('status' => 400));
        }
        ...
        if (!self::is_uuid($checkout_id) || !self::is_uuid($anonymous_id) || !self::verify_capture_token($capture_token, $anonymous_id, $checkout_id)) {
            return new WP_Error('adoology_capture_invalid', __('Invalid checkout identity.', 'adoology-connector'), array('status' => 400));
        }

        $result = self::store_snapshot($checkout_id, array(
            'anonymous_id' => $anonymous_id,
            'session_id'   => isset($params['session_id']) ? $params['session_id'] : '',
            'flow'         => isset($params['flow']) ? $params['flow'] : 'checkout',
            'product_id'   => isset($params['product_id']) ? $params['product_id'] : 0,
            'variation_id' => isset($params['variation_id']) ? $params['variation_id'] : 0,
            'quantity'     => isset($params['quantity']) ? $params['quantity'] : 1,
            'value_minor'  => isset($params['value_minor']) ? $params['value_minor'] : 0,
            'currency'     => isset($params['currency']) ? $params['currency'] : get_woocommerce_currency(),
            'customer'     => isset($params['customer']) && is_array($params['customer']) ? $params['customer'] : array(),
            'landing_page' => isset($params['landing_page']) ? $params['landing_page'] : '',
            'form_stage'   => isset($params['form_stage']) ? $params['form_stage'] : 'started',
        ...
        return new WP_REST_Response(array('stored' => true), 202);
    }
    ...
    public static function store_snapshot($checkout_id, $data) {
        ...
        if ($existing) {
            if (in_array($existing['status'], array('converted', 'recovered', 'expired'), true)) {
                return true;
            }
            $stages = array('started' => 1, 'details' => 2, 'leaving' => 3, 'submitted' => 4, 'completed' => 5);
        ...
        if (!$inserted) {
            ...
            if ($race) {
                ...
                $wpdb->update($table, $row, array('id' => (int) $race));
                return true;
            }
    ...
    public static function store_api_capture_identity($order, $request) {
        ...
        $extension   = is_array($extensions) && isset($extensions['adoology']) && is_array($extensions['adoology']) ? $extensions['adoology'] : array();
    ...
    public static function mark_complete($checkout_id, $order_id, $order = null) {
        ...
        $status = in_array($row['status'], array('incomplete', 'submitting_recovery'), true) ? 'recovered' : 'converted';
    ...
    public static function claim_submission($checkout_id) {
        ...
        if (in_array($row['status'], array('converted', 'recovered'), true) && (int) $row['order_id'] > 0) {
            return array('claimed' => false, 'order_id' => (int) $row['order_id']);
        }
        if (!in_array($row['status'], array('started', 'incomplete'), true)) {
        ...
        $updated = $wpdb->update($table, array('status' => $next, 'updated_at' => gmdate('Y-m-d H:i:s')), array('id' => (int) $row['id'], 'status' => $row['status']));
        return $updated ? array('claimed' => true, 'order_id' => 0) : new WP_Error('adoology_checkout_processing', __('This order submission is already being processed.', 'adoology-connector'));
    }
    ...
    public static function to_minor($amount) {
        return max(0, (int) round((float) $amount * pow(10, wc_get_price_decimals())));
    }
    ...
    public static function register_exporter($exporters) {
        $exporters['adoology-incomplete-orders'] = array(
    ...
    public static function register_eraser($erasers) {
        $erasers['adoology-incomplete-orders'] = array(
    ...
    public static function export_personal_data($email, $page = 1) {
        ...
        foreach ($matches['rows'] as $match) {
            ...
            foreach ($match['customer'] as $name => $value) {
                $fields[] = array('name' => ucfirst(str_replace('_', ' ', $name)), 'value' => is_array($value) ? wp_json_encode($value) : $value);
            }
            foreach (array('checkout_id', 'session_id', 'flow', 'status', 'product_id', 'variation_id', 'quantity', 'value_minor', 'currency', 'landing_page', 'form_stage', 'risk_score', 'order_id', 'created_at', 'updated_at') as $field) {
                $fields[] = array('name' => ucfirst(str_replace('_', ' ', $field)), 'value' => $match['row'][$field]);
            }
            foreach (self::events_for_anonymous($match['customer']['anonymous_id'] ?? '') as $event) {
                $fields[] = array('name' => __('Adoology event', 'adoology-connector'), 'value' => wp_json_encode($event));
            }
            $data[] = array(
        ...
        return array('data' => $data, 'done' => $matches['done']);
    }
    ...
    public static function erase_personal_data($email, $page = 1) {
        ...
        return array(
            ...
            'messages'       => $removed ? array(__('Local data was erased. Data already delivered to Adoology must be erased from the connected workspace separately.', 'adoology-connector')) : array(),
            'done'           => true,
        );
    }
    ...
    private static function privacy_rows($email, $page, $limit = 100) {
        ...
        return array('rows' => $matches, 'done' => $limit === 0 || count($rows) < $limit);
    }
