...
class Adoology_Webhooks {
    ...
    /** @var array */
    private static $reveal_secret_once = array();

    /** @var array */
    private static $deletion_types = array();

    /** @var array */
    private static $variation_parents = array();

    /** @var array */
    private static $routed_post_actions = array();
    ...
    public static function register() {
        add_filter('woocommerce_webhook_topic_hooks', array(__CLASS__, 'filter_topic_hooks'), 20, 2);
        add_filter('woocommerce_webhook_should_deliver', array(__CLASS__, 'filter_should_deliver'), 20, 3);
        add_filter('woocommerce_webhook_payload', array(__CLASS__, 'filter_payload'), 20, 4);
        add_filter('woocommerce_webhook_http_args', array(__CLASS__, 'filter_http_args'), 20, 3);
        add_filter('woocommerce_webhook_secret', array(__CLASS__, 'filter_webhook_secret'), 20, 2);
        add_action('woocommerce_webhook_delivery', array(__CLASS__, 'handle_delivery'), 20, 5);
        add_action(self::RETRY_HOOK, array(__CLASS__, 'retry_delivery'), 10, 1);

        add_action('before_delete_post', array(__CLASS__, 'capture_before_delete'), 1, 1);
        add_action('wp_trash_post', array(__CLASS__, 'capture_before_trash'), 1, 1);
        add_action('woocommerce_before_delete_product_variation', array(__CLASS__, 'capture_variation_parent'), 1, 1);
    }
    ...
    public static function filter_topic_hooks($hooks, $webhook) {
        ...
        if (isset($hooks['product.created'])) {
            $hooks['product.created'] = array_values(array_diff($hooks['product.created'], array('woocommerce_new_product_variation')));
        }

        if (isset($hooks['product.updated'])) {
            $hooks['product.updated'] = array_values(array_unique(array_merge($hooks['product.updated'], array(
        ...
        if (isset($hooks['product.deleted'])) {
            $hooks['product.deleted'] = array_values(array_unique(array_merge($hooks['product.deleted'], array(
                'before_delete_post',
                'delete_post',
            ))));
        }
    ...
    public static function filter_should_deliver($should, $webhook, $arg) {
        ...
        if ($type === 'product_variation') {
            ...
            $post_actions = array('wp_trash_post', 'delete_post', 'untrashed_post');
        ...
        if ($type === 'product' && $topic === 'product.updated' && in_array($hook, array('wp_trash_post', 'delete_post', 'before_delete_post'), true)) {
            return false;
        }
    ...
    public static function handle_delivery($http_args, $response, $duration, $arg, $webhook_id) {
        ...
        $headers  = isset($http_args['headers']) && is_array($http_args['headers']) ? $http_args['headers'] : array();
    ...
    public static function retry_delivery($event_id) {
        ...
        $response  = wp_safe_remote_request($webhook->get_delivery_url(), array(
            'method'              => 'POST',
            'timeout'             => 20,
            'redirection'         => 0,
            ...
            'headers'             => array(
                ...
                'X-WC-Webhook-Resource'           => isset($topic_bits[0]) ? $topic_bits[0] : 'product',
                'X-WC-Webhook-Event'              => isset($topic_bits[1]) ? $topic_bits[1] : 'updated',
    ...
    private static function product_snapshot($product) {
        ...
        $data   = array(
            ...
            'grouped_products'   => method_exists($product, 'get_children') && $product->is_type('grouped') ? array_map('intval', $product->get_children('edit')) : array(),
    ...
    private static function mapping_meta($product) {
        $meta = array();
        foreach (array('_adoology_master_product_id', '_adoology_master_variant_id') as $key) {
            $value = $product->get_meta($key, true, 'edit');
            if (is_scalar($value) && (string) $value !== '') {
                $meta[] = array('key' => $key, 'value' => (string) $value);
            }
        }
        return $meta;
    }
    ...
    private static function parent_attributes($product) {
        ...
        foreach ($product->get_attributes('edit') as $attribute) {
            ...
            $options = $attribute->is_taxonomy() ? array_map(function ($term) {
                return (string) $term->name;
            }, $attribute->get_terms()) : array_values(array_map('strval', $attribute->get_options()));
    ...
    private static function capture_deletion_context($post_id) {
        ...
        set_transient(self::deletion_context_key($post_id), array(
            'type'      => $post->post_type,
