...
class Adoology_API_Client {
    ...
    public static function request($method, $path, $body = null, $idempotency_key = '', $max_attempts = self::MAX_ATTEMPTS, $timeout = 20) {
        ...
        $writes = array('POST', 'PATCH', 'DELETE');
        ...
        for ($attempt = 1; $attempt <= $max_attempts; $attempt++) {
            $response = wp_safe_remote_request($url, array(
                ...
                'httpversion'         => '1.1',
                'user-agent'          => 'Adoology-Connector/' . ADOOLOGY_VERSION . '; ' . home_url('/'),
                'headers'             => $headers,
                'body'                => $encoded_body,
            ));
    ...
    public static function ingest_events($events, $idempotency_key) {
        return self::request('POST', '/events', array('events' => array_values($events)), $idempotency_key, 1, 10);
    }
    ...
    private static function retry_delay_microseconds($attempt, $response) {
        $seconds = min(2, (int) pow(2, max(0, $attempt - 1)));
        if (!is_wp_error($response)) {
