...
class RestController {
    ...
    public static function register() {
        add_action('rest_api_init', function () {
            register_rest_route('adoology/v1', '/stock', array(
                'methods'             => WP_REST_Server::CREATABLE,
                'callback'            => array(__CLASS__, 'handle_stock'),
    ...
    public static function verify_signature(WP_REST_Request $request) {
        ...
        if ($secret === '') {
            return new WP_Error('adoology_not_configured', __('No inbound signing secret configured.', 'adoology-connector'), array('status' => 503));
        }
        ...
        if (!hash_equals($expected, $signature)) {
            return new WP_Error('adoology_bad_signature', __('Invalid webhook signature.', 'adoology-connector'), array('status' => 401));
        }
    ...
    public static function handle_stock(WP_REST_Request $request) {
        ...
        if (!is_array($data)) {
            return new WP_Error('adoology_bad_payload', __('Missing data payload.', 'adoology-connector'), array('status' => 422));
        }
        ...
        if ($quantity === null) {
            return new WP_Error('adoology_bad_quantity', __('data.quantity must be numeric.', 'adoology-connector'), array('status' => 422));
        }

        $product = self::resolve_product($data);
        if (!$product instanceof WC_Product) {
            return new WP_Error('adoology_product_not_found', __('No product matches the payload (sku / product_id / variation_id).', 'adoology-connector'), array('status' => 404));
        }

        wc_update_product_stock($product, $quantity, 'set');

        $product->update_meta_data('_adoology_stock_sync', array(
            'quantity'    => $quantity,
            'event'       => $event,
